...
<html>
    <head>
        <%
            ...
            Photo userPhoto = photoDAO.getPhotoByUserID(user.getID());
            Photo userPhotoInstance = photoDAO.getPhotoByUserID(userInstance.getID());
            ArrayList<Poster> userPosts = posterDAO.getPostersbyUserID(user.getID());
            ArrayList<String> listTopicID = topicDAO.getListTopicIDByDESC();
        %>
        <title><%=user.getDisplayname()%> - My Posts</title>
        <meta charset="utf-8">
    ...
    <body>
        ...
        <div id="colorlib-page">
            ...
            <aside id="colorlib-aside" role="complementary" class="js-fullheight text-center">
                <br><br><h1 id="colorlib-logo"><a>Posts<span>.</span></a></h1>
                <nav id="colorlib-main-menu" role="navigation">
                    <ul>
                        <li ><a href="aboutMe?uid=<%=user.getID()%>">About Me</a></li>
                        <li class="colorlib-active"><a href="myPosts?uid=<%=user.getID()%>">My Posts</a></li>
                            <%
                                if (user.getID().equals(userInstance.getID())) {
                            %>
                        <li><a href="profileSettings?uid=<%=user.getID()%>">BookWorm Settings  >> </a></li>
                            <%}%>
                    </ul>
                </nav>
            ...
            <div id="colorlib-main">
                <section class="ftco-section">
                    <div class="container">
                        ...
                        <div class="row">
                            <%
                                for (Poster p : userPosts) {
                                    int commentCount = commentDAO.getCommentCountByPostID(p.getID());
                                    ArrayList<Topic> listTopics = topicDAO.getTopicsByPostID(p.getID());
                            %>
                            <div class="col-md-4">
                                <div class="blog-entry ftco-animate">
                                    ...
                                    <div class="text text-2 pt-2 mt-3">
                                        ...
                                        <p class="mb-4"><%=(p.getText().length() >= 50) ? p.getText().substring(0, 50) : p.getText() + "..."%></p>
                                        <div class="author mb-4 d-flex align-items-center">
                                            <a href="aboutMe?uid=<%=user.getID()%>" class="img" style="background-image: url(images/<%=userPhoto.getPhotoname()%>);"></a>
                                            <div class="ml-3 info">
                                                <span>Written by</span>
                                                <h3><a href="aboutMe?uid=<%=user.getID()%>"><%=user.getDisplayname()%></a>, <span><%=p.getPostingDate()%></span></h3>
                                            </div>
                                        </div>
                                    ...
                                    <ul class="list-unstyled categories">
                                        <%
                                            ...
                                            for (int k = listTopicID.size() - 1; k >= 0; k--) {
                                                Topic topic = topicDAO.getTopicbyTopicID(listTopicID.get(k));
                                ...
                                <div class="ftco-footer-widget mb-4">
                                    ...
                                    <div class="block-23 mb-3">
                                        <ul>
                                            ...
                                            <li><a><span class="icon icon-envelope"></span><span class="text">quanghhhe130699@fpt.edu.vn</span></a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
